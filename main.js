...
pageTurnBtn.forEach((el, index) => {
    el.onclick = () => {
        // Check if we're on mobile (book layout is disabled)
...
contactMeBtn.onclick = () => {
    ...
    if (window.innerWidth <= 768) {
        const contactSection = document.querySelector('.page-back .title');
        if (contactSection) {
...
backProfileButton.onclick = () => {
    // On mobile, just scroll to profile section
    if (window.innerWidth <= 768) {
...
// Only run book animation on desktop
if (window.innerWidth > 768) {
